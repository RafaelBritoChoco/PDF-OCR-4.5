...
// Re-export specific utils for App.tsx usage if needed
export { 
    guardStep2ContentIntegrity, 
    convertShortTagsToFullStructure, 
    validateStructuralIntegrity, 
...
// Helper: split text into chunks (used in comparisons)
    ...
    try {
      ...
      while (retries > 0 && !success) {
        try {
          ...
          if (retries > 1) {
            await delay(isRateLimit ? 5000 * (4 - retries) : 2000);
            retries--;
            onLog?.(`Retrying page ${pageNumber}...`);
          } else {
            throw err;
          }
        }
...
export const processDocumentChunk = async (options: ProcessChunkOptions): Promise<string> => {
  ...
  const currentTimeout = timeoutMs || (model.includes("pro") ? PRO_TIMEOUT_MS : DEFAULT_TIMEOUT_MS);
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      if (attempt > 1) {
          ...
          onLog?.(`Sending request to AI (${model})...`);
      }
      ...
      heartbeat = setInterval(() => {
          secondsElapsed += 5;
          if (secondsElapsed >= 5) {
              onLog?.(`... AI is processing (${secondsElapsed}s elapsed)`);
          }
      }, 5000);
      ...
      const isInternalError = error.status === 500 || error.message?.includes("500");

      if (isInternalError || error.message?.includes("TIMEOUT") || isRateLimit || isValidationFailed) {
        if (attempt < MAX_RETRIES) {
          const baseDelay = isRateLimit ? 5000 : 2000;
          const delayMs = baseDelay * Math.pow(2, attempt - 1) + Math.random() * 500;
          onLog?.(`⚠️ Error/Retry. Pausing for ${(delayMs/1000).toFixed(1)}s...`);
          await delay(delayMs);
          continue;
        }
